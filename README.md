...
- 🎯 Entry points to execute your program using `python -m <myfastapi>` or `$ myfastapi` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/qinlu1024/myfastapi/branch/main/graph/badge.svg?token=myfastapi_token_here)](https://codecov.io/gh/qinlu1024/myfastapi)
[![CI](https://github.com/qinlu1024/myfastapi/actions/workflows/main.yml/badge.svg)](https://github.com/qinlu1024/myfastapi/actions/workflows/main.yml)

Awesome myfastapi created by qinlu1024

## Install

from source
```bash
git clone https://github.com/qinlu1024/myfastapi myfastapi
cd myfastapi
make install
```
...
```bash
pip install myfastapi
```

## Executing

```bash
$ myfastapi run --port 8080
```

or

```bash
python -m myfastapi run --port 8080
```
...
```bash
❯ myfastapi --help
Usage: myfastapi [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ myfastapi create-user --help
Usage: myfastapi create-user [OPTIONS] USERNAME PASSWORD
...
```bash
myfastapi create-user admin admin --superuser
```
...
Run with `myfastapi run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `myfastapi create-user`.
...
-----------------------------------------------------
myfastapi/__init__.py              4      0   100%
myfastapi/app.py                  16      1    94%
myfastapi/cli.py                  21      0   100%
myfastapi/config.py                5      0   100%
myfastapi/db.py                   10      0   100%
myfastapi/models/__init__.py       0      0   100%
myfastapi/models/content.py       47      1    98%
myfastapi/routes/__init__.py      11      0   100%
myfastapi/routes/content.py       52     25    52%
myfastapi/routes/security.py      15      1    93%
myfastapi/routes/user.py          52     26    50%
myfastapi/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from myfastapi.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in myfastapi/default.toml.

### As environment variables
```bash
export myfastapi_KEY=value
export myfastapi_KEY="@int 42"
export myfastapi_KEY="@jinja {{ this.db.uri }}"
export myfastapi_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
myfastapi_ENV=production myfastapi run
```
